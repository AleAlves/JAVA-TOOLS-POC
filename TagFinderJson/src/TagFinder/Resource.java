...
class Resource implements Files {
	...
	public List<String> searchTag(Tag[] tagList, List<File> dir, boolean onlyNotFound) {
        ...
        for (int j = 0; j < tagList.length; j++) {
            ...
            for (int k = 0; k < dir.size(); k++) {
                ...
                while (input.hasNextLine()) {
                    ...
                    if (line.contains(tagList[j].getNomedaMetrica().toString().toLowerCase()) ) {
                    	if(!onlyNotFound)
//                    		foundtags.add("- Familia: "+ tagList[j].getFamiliadeProdutoServicosSantander() +" Subfamilia: "+ tagList[j].getSubfamilia() + " Métrica: "+tagList[j].getNomedaMetrica() + " found at "+ dir.get(k) + ", line: "+ linesCount);
                    	isFound = true;
						break;
                    }
                    linesCount++;
            ...
            if(!isFound){
            	System.out.println("- Familia: "+ tagList[j].getFamiliadeProdutoServicosSantander() +" Subfamilia: "+ tagList[j].getSubfamilia() + " Métrica: "+tagList[j].getNomedaMetrica() +" not found anywhere...");
            }
            input.close();
        }
        return foundtags;
